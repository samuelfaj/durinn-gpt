...
Sua missão é criar uma migration usando a biblioteca Sequelize ORM. Essa migration deve usar transaction e você deve responder em markdown apenas o código novo entre (\`\`\`). Sem explicações.`;
	...
	public static description = `Gera uma migration do Sequelize a partir das instruções que passamos, use o padrão camelCase e adicione comentários nas colunas sempre que possível. Exemplo: Crie uma migration adicionando o campo nome (string) na tabela Usuários`;
	...
	static async run(codeOrFile: string, saveToFile ?: string, verbose = false){
		...
		return super.run(codeOrFile, saveToFile, verbose);
	}
}
